...
/* --- AJOUTS POUR LE FILTRE DE KALMAN --- */
// Calibration : Ajustez cette valeur pour corriger l'erreur constante du RTT
#define RTT_OFFSET_M          (0.0f) 
#define ifft_OFFSET_M 		  (0.6f)  

// Paramètres du Filtre (Tuning)
#define KALMAN_Q_PROCESS      (0.01f)  // Vitesse de changement supposée de la cible (faible = mouvement fluide)
#define KALMAN_R_RTT          (3.0f)   // Bruit estimé du RTT (Grand = on ne lui fait pas confiance)
#define KALMAN_R_IFFT         (0.2f)   // Bruit estimé de l'IFFT (Petit = on lui fait confiance)
/* -------------------------------------- */
...
#define DE_SLIDING_WINDOW_SIZE (1)
...
typedef struct {
    float x;  // État actuel (Distance estimée finale)
    float p;  // Estimation de l'erreur (Covariance)
    bool initialized;
} kalman_state_t;

// Une instance de filtre par antenne (AP)
static kalman_state_t k_filters[MAX_AP];

void kalman_init(kalman_state_t *k, float initial_value)
{
    k->x = initial_value;
    k->p = 1.0f; // Incertitude initiale
    k->initialized = true;
}

void kalman_update(kalman_state_t *k, float measurement, float measurement_noise_r)
{
    if (!isfinite(measurement)) {
        return; // On ignore les erreurs de mesure (NAN)
    }

    // 1. Prédiction (On augmente l'incertitude car le temps a passé)
    k->p = k->p + KALMAN_Q_PROCESS;

    // 2. Mise à jour (Correction avec la nouvelle mesure)
    float k_gain = k->p / (k->p + measurement_noise_r);
...
int main(void)
{
	...
	struct bt_le_cs_create_config_params config_params = {
		...
		.min_main_mode_steps = 30,
		.max_main_mode_steps = 30,
	...
	const struct bt_le_cs_set_procedure_parameters_param procedure_params = {
		...
		.snr_control_initiator = BT_LE_CS_SNR_CONTROL_18dB,
		.snr_control_reflector = BT_LE_CS_SNR_CONTROL_18dB,
	};
    ...
    // Initialisation des filtres à "non initialisé"
    for(int i=0; i<MAX_AP; i++) k_filters[i].initialized = false;
    ...
    while (true) {
        ...
        for (uint8_t ap = 0; ap < MAX_AP; ap++) {
            ...
            // 1. Récupération des médianes
            cs_de_dist_estimates_t dist_raw = get_distance(ap);

            // Application des offsets
            float rtt_calibrated = dist_raw.rtt + RTT_OFFSET_M;
            float ifft_calibrated = dist_raw.ifft - ifft_OFFSET_M;
            
            // --- CHANGEMENT 1 : Initialisation intelligente ---
            // On n'initialise QUE si on a un RTT physique crédible (> 0.2m)
            if (!k_filters[ap].initialized && isfinite(rtt_calibrated) && dist_raw.rtt > 0.20f) {
                kalman_init(&k_filters[ap], rtt_calibrated);
                LOG_INF("Filtre Kalman initialise sur AP %d a %.2fm", ap, (double)rtt_calibrated);
            } 
            // Si le RTT est nul mais que l'IFFT est bon, on peut aussi initialiser sur l'IFFT
            else if (!k_filters[ap].initialized && isfinite(ifft_calibrated)) {
                kalman_init(&k_filters[ap], ifft_calibrated);
                LOG_INF("Filtre Kalman initialise sur IFFT (AP %d) a %.2fm", ap, (double)ifft_calibrated);
            }

            if (k_filters[ap].initialized) {
                // --- CHANGEMENT 2 : Fusion RTT Sécurisée ---
                // On n'envoie au Kalman que si la valeur n'est pas un "zéro de bruit"
                if (isfinite(rtt_calibrated) && dist_raw.rtt > 0.20f) {
                    kalman_update(&k_filters[ap], rtt_calibrated, KALMAN_R_RTT);
                }

                // --- ÉTAPE B : Fusion IFFT (Toujours active si valide) ---
                if (isfinite(ifft_calibrated)) {
                    kalman_update(&k_filters[ap], ifft_calibrated, KALMAN_R_IFFT);
                }

                // --- Affichage du résultat final ---
                LOG_INF("AP %d | RTT Calibre: %.2fm | IFFT: %.2fm | => FUSION KALMAN: %.3f meters <<<",
                        ap, 
                        (double)rtt_calibrated,
                        (double)ifft_calibrated, 
                        (double)k_filters[ap].x); 
